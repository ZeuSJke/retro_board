...
### AI (`app/ai/`)
- **`ai_client.py`**: shared **synchronous** httpx client against the self-hosted LiteLLM gateway (OpenAI-compatible `/v1`). Base URL/key/model come from `Settings` (`AI_BASE_URL`, `AI_API_KEY`, `AI_MODEL`; see `config.py`). `ai_client.generate()` blocks the event loop — **always wrap calls in `asyncio.to_thread()` from async endpoints**. May raise `RuntimeError`; catch broadly.
- **Default model**: `settings.ai_model` (`qwen3.6-35b-a3b-mtp-ud-q8_k_xl`). All call sites — `ai_client.py` default, `summary_generation.py`, `title_generation.py`, `clustering.py` — use the same model from settings; change it in one env var `AI_MODEL`.
- **Qwen3 is a reasoning model**: without `chat_template_kwargs: {"enable_thinking": false}` (sent when `AI_DISABLE_THINKING=true`, default) the whole `max_tokens` budget goes into `reasoning_content` and `content` comes back empty → `generate()` raises. If AI features return empty results, check `docker compose logs backend | grep -i "AI request failed"` for 401/404/timeout first.
