...
def create_workspace_token(workspace_id: str, workspace_slug: str, workspace_name: str) -> str:
    """Создать JWT токен для workspace."""
    expire = datetime.utcnow() + timedelta(hours=settings.workspace_jwt_expire_hours)
    payload = {
...
def get_current_workspace(
    ...
    """Dependency: получить текущий workspace из headers."""
    token = request.headers.get("X-Workspace-Token")
    if not token:
        raise HTTPException(status_code=401, detail="X-Workspace-Token header required")
